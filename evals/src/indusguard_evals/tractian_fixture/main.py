...
app = FastAPI(
    ...
    description=(
        ...
        "Ações de impacto exigem justificativa (>=20 chars). "
        "Contexto de usuário via header x-user-id."
    ),
)
...
def _seed(
    seed: str | None = Query(default=None, description="Semente para comportamento probabilístico"),
) -> str | None:
    return seed
...
# ---------------------------------------------------------------------------
def _apply_mode(
    payload: dict[str, Any], mode: Mode, category: str
...
def list_analyses(
    asset_id: str, status: str | None = Query(default=None), seed: str | None = Depends(_seed)
):
    if not store.get_asset(asset_id):
        raise HTTPException(404, "Ativo não encontrado.")
...
def get_baseline(
    asset_id: str, point_id: str | None = Query(default=None), seed: str | None = Depends(_seed)
):
    if not store.get_asset(asset_id):
        raise HTTPException(404, "Ativo não encontrado.")
    ...
    if baseline is None:
        return envelope(
            {"baseline": None}, Mode.INCONCLUSIVE, "Nenhum baseline para este ativo/ponto."
        )
...
def get_rms(
    ...
    payload = {
        ...
        "unit": "mm/s",
        "baseline_reference": (baseline.get("features") or [{}])[0].get("reference")
        if baseline
        else None,
...
def search_knowledge(
    q: str = Query(...), type: str | None = Query(default=None), seed: str | None = Depends(_seed)
):
