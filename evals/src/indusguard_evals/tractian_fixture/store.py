...
def configure_data_dir(path: Path) -> None:
    """Aponta a fixture para o snapshot versionado e invalida caches de outra execução."""

    global DATA_DIR
    DATA_DIR = path.resolve()
    _tables.cache_clear()
    seed_config.cache_clear()
...
def search_knowledge(q: str, type_: str | None = None) -> list[dict[str, Any]]:
    ...
    ql = _normalize(q)
    mask = df["title"].map(_normalize).str.contains(ql) | df["body"].map(_normalize).str.contains(
        ql
    )
    hits = df[mask]
    if type_:
